...
def test_lead_catalog_is_fifty_real_and_unique():
    ...
    assert len(wave3) == 62
    assert all(row.get("email") for row in wave3), "La 3e vague privée/OBNL doit toutes avoir un courriel public vérifié."
    ...
    private_sectors = {row["sector"] for row in wave3}
    assert len(private_sectors) >= 8, "La vague privée doit couvrir plusieurs secteurs."
