...
from app.models import Application, Candidate, Company, JobOffer, Message, User
...
def _is_staff(user: User) -> bool:
    return user.role in {UserRole.RECRUITER, UserRole.ADMIN, UserRole.FINANCE}
...
def _can_converse(db: Session, sender: User, recipient: User) -> bool:
    ...
    if not recipient.is_active:
        return False
    if _is_staff(sender) and recipient.role in {UserRole.CANDIDATE, UserRole.EMPLOYER, UserRole.RECRUITER, UserRole.ADMIN, UserRole.FINANCE}:
        return True
    ...
    if sender.role == UserRole.EMPLOYER and recipient.role == UserRole.CANDIDATE:
        company = db.scalar(select(Company).where(Company.owner_user_id == sender.id))
        if not company:
            return False
        return (
            db.scalar(
                ...
                .where(JobOffer.company_id == company.id, Candidate.user_id == recipient.id)
            )
            is not None
        )
...
def directory(db: Session, user: User) -> list[dict]:
    ...
    if user.role == UserRole.CANDIDATE:
        people = list(db.scalars(select(User).where(User.role.in_([UserRole.RECRUITER, UserRole.ADMIN]), User.is_active.is_(True))).all())
        ...
        people = list(
            db.scalars(
                select(User).where(
                    ...
                    User.role.in_([UserRole.CANDIDATE, UserRole.EMPLOYER, UserRole.RECRUITER, UserRole.ADMIN]),
                    User.id != user.id,
                )
            ).all()
        )
