...
from app.models import Company, Invoice, Payment, RecruitmentMission, User
...
def _load(db: Session, invoice_id: str) -> Invoice | None:
    return db.scalar(
        select(Invoice)
        .options(joinedload(Invoice.company), joinedload(Invoice.mission), selectinload(Invoice.payments))
        .where(Invoice.id == invoice_id)
    )


def _visible(db: Session, user: User, row: Invoice) -> bool:
    if user.role in {UserRole.ADMIN, UserRole.FINANCE, UserRole.RECRUITER}:
        return True
...
def list_invoices(db: Session, user: User) -> list[Invoice]:
    stmt = select(Invoice).options(joinedload(Invoice.company), joinedload(Invoice.mission), selectinload(Invoice.payments))
    if user.role == UserRole.EMPLOYER:
        company = db.scalar(select(Company).where(Company.owner_user_id == user.id))
        if not company:
            return []
        stmt = stmt.where(Invoice.company_id == company.id)
    elif user.role not in {UserRole.ADMIN, UserRole.FINANCE, UserRole.RECRUITER}:
